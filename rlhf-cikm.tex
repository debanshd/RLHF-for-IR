...
\begin{figure*}[t]
  ...
  \caption{The Sparse Semantic Steering pipeline. A frozen dense encoder produces $E_q$, which is projected through an SAE bottleneck into sparse facets. The PPO agent receives a context-aware state (top-$k$ active facets from PRF), outputs a bi-directional steering action $\Delta M$, and the corrected query is re-ranked against the corpus index. The reward is computed from rank changes during training only.}
...
\bibitem{geva2022}
Mor Geva, Avi Caciularu, Kevin Robinson, and Yoav Goldberg. 2022. Transformer Feed-Forward Layers Build Predictions by Promoting Concepts in the Vocabulary Space. In \textit{Proceedings of EMNLP}.
