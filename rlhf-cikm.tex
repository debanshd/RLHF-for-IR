...
While dense retrieval has become the dominant approach in modern information retrieval, superseding the reliance on purely lexical matching by utilizing learned vector representations~\cite{karpukhin2020, izacard2022}, dense embeddings can struggle to distinguish fine grained semantic differences, making models vulnerable to hard negatives~\cite{zhan2021}: furthermore, they often fail to generalize across diverse domains, experiencing significant performance drops in zero shot settings due to domain shift~\cite{thakur2021}. Correcting this has historically required expensive offline fine tuning over hard negative distributions, a reactive strategy that scales poorly.
...
\textbf{Action Space ($\mathcal{A}$):} The agent outputs $\Delta M \in [-2.0, 2.0]^{128}$, enabling bidirectional Negative Semantic Masking. The sparse action is projected back by the SAE decoder weights, scaled by a Rank Proportional torque:
